...
export function createRenderer({ statePath, artPacksDir }) {
  ...
  function renderFrame(frameBuffer) {
    const { cols, rows } = getTermSize();
    const bubbleOffset = currentBubbleLines > 0 ? currentBubbleLines + 1 : 0;
    const spriteStartRow = 2 + bubbleOffset;
    const statsAreaRows = 12;
    const charRows = rows - spriteStartRow - statsAreaRows;
    const base64 = frameBuffer.toString("base64");
    write(moveCursorTo(spriteStartRow, 2));
    write(buildDeleteSequence(IMAGE_ID));
    write(buildTransmitSequence(base64, { cols: cols - 4, rows: Math.max(charRows, 4), imageId: IMAGE_ID }));
  }
  ...
  function renderStats(state) {
    ...
    const bar = (value, color) => {
      ...
      return color + "\u2588".repeat(filled) + RESET + "\u2591".repeat(16 - filled);
    };
    ...
    const dividerRow = rows - 11;
    renderDivider(dividerRow, buddy.name || "Buddy");
    ...
    for (let i = 0; i < lines.length; i++) {
      write(moveCursorTo(dividerRow + 1 + i, 3));
      write("\x1b[2K" + BOLD + "\u2551" + RESET + " " + lines[i]);
      write(moveCursorTo(dividerRow + 1 + i, cols));
      write(BOLD + "\u2551" + RESET);
    }
  }
